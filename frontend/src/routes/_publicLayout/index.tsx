  ...
  return (
    <section className="mt-8 px-2 sm:px-0">
      ...
      {isLoading ? (
        ...
        <>
          {/* Zone 1: Featured pair — compact with images */}
          {featuredArticles.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              {featuredArticles.map((article, i) => {
                const { title, imageUrl, dateStr, plain, slug, author } = articleMeta(article);
                return (
                  <FeaturedArticleCard key={article.article.id} title={title} imageUrl={imageUrl} dateStr={dateStr} excerpt={plain} slug={slug} author={author} index={i} />
                );
              })}
            </div>
          )}
            ...
            <div className="rounded-xl bg-black/20 backdrop-blur-sm border border-white/[0.05] overflow-hidden">
              {listArticles.map((article, i) => {
                ...
                return (
                  ...
                  >
                    <div className="w-12 h-9 shrink-0 rounded-lg overflow-hidden bg-white/[0.03]">
                      {imageUrl ? (
                        <img src={imageUrl} alt="" className="w-full h-full object-cover transition-transform duration-200 group-hover:scale-105" />
                      ) : (
...
function FeaturedArticleCard({ title, imageUrl, dateStr, excerpt, slug, author, index }: { title: string; imageUrl?: string; dateStr: string; excerpt: string; slug: string; author?: string; index: number }) {
  ...
  return (
    ...
    >
      <div className="relative w-20 sm:w-24 shrink-0 aspect-[4/3] overflow-hidden rounded-l-2xl">
        {imageUrl ? (
          <img src={imageUrl} alt="" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
        ) : (
      ...
      <div className="flex-1 p-3 flex flex-col min-w-0">
        <h3 className="text-sm font-semibold tracking-tight text-white group-hover:text-primary transition-colors line-clamp-2">{title}</h3>
        <p className="text-[11px] text-white/35 line-clamp-1 mt-0.5">{excerpt}</p>
        <div className="flex items-center gap-2 text-[10px] text-white/20 mt-1.5">
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {[0, 1].map((i) => (
          <div key={i} className="rounded-2xl bg-white/[0.03] border border-white/[0.05] overflow-hidden animate-pulse flex flex-row">
            <div className="w-20 sm:w-24 aspect-[4/3] shrink-0 bg-white/[0.06]" />
      ...
      <div className="rounded-xl bg-white/[0.02] border border-white/[0.05] overflow-hidden">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="flex items-center gap-3 px-3 py-2.5 border-b border-white/[0.03] animate-pulse">
            <div className="w-12 h-9 shrink-0 bg-white/[0.06] rounded-lg" />
