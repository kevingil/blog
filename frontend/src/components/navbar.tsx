...
export const Navbar = () => {
  ...
  return (
    <header
      ...
      style={{
        background: atTop ? 'transparent' : `color-mix(in oklch, var(--background) ${Math.round(88 * progress)}%, transparent)`,
        ...
        borderBottom: atTop ? '1px solid transparent' : `1px solid color-mix(in oklch, var(--border) ${Math.round(100 * progress)}%, transparent)`,
        boxShadow: atTop ? 'none' : '0 12px 48px -24px color-mix(in oklch, var(--foreground) 22%, transparent)',
        transition: 'background 0.8s cubic-bezier(0.4, 0, 0.2, 1), border-color 0.8s cubic-bezier(0.4, 0, 0.2, 1), backdrop-filter 0.8s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.8s cubic-bezier(0.4, 0, 0.2, 1)',
      }}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
        ...
        <Link
          to="/"
          className="flex items-center px-2 py-1.5 -mx-2 rounded-lg hover:bg-accent transition-colors"
        >
          <GlitchText
            text="KG"
        ...
        {/* Nav - desktop */}
        <nav className="hidden lg:flex flex-nowrap items-center gap-1 whitespace-nowrap">
          {routeList.map(({ href, label }) => (
            <Link
              key={href}
              to={href}
              className="px-4 py-2 rounded-lg text-sm font-medium text-foreground/70 hover:text-foreground hover:bg-accent transition-all duration-200"
            >
              {label}
            </Link>
          ))}
        ...
        <div className="flex lg:hidden items-center">
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent
              side="right"
              className="flex flex-col justify-between bg-background/95 backdrop-blur-2xl border-l border-border text-foreground"
            >
              <div>
                <SheetHeader className="mb-6 ml-4">
                  <SheetTitle className="flex items-center">
                    <Link to="/" className="flex items-center">
                      <GlitchText
                        text="KG"
                        className="text-xl font-bold font-gloria text-foreground"
                      />
                    </Link>
                  </SheetTitle>
                </SheetHeader>
                ...
                <div className="flex flex-col gap-1 px-2">
                  {routeList.map(({ href, label }) => (
                    <Button
                      ...
                      asChild
                      variant="ghost"
                      className="justify-start text-base text-foreground/70 hover:text-foreground hover:bg-accent rounded-xl"
                    >
          ...
          <ToggleTheme showLabel onClick={() => setIsOpen(false)} />
              </SheetFooter>
            </SheetContent>
          </Sheet>
        </div>
