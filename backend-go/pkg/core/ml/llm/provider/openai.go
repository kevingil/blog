...
func outputMessageID(msg message.Message, index int) string {
	if msg.ID != "" {
		if strings.HasPrefix(msg.ID, "msg_") {
			return msg.ID
		}
		return "msg_" + strings.ReplaceAll(msg.ID, "-", "_")
	}
	return fmt.Sprintf("msg_history_%d", index)
}
