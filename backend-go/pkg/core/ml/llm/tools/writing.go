...
func (t *ReadDocumentTool) Run(ctx context.Context, params ToolCall) (ToolResponse, error) {
	...
	if docContent == "" {
		log.Printf("📖 [ReadDocument] Returning empty document")
...
func (t *ReplaceLinesTool) Run(ctx context.Context, params ToolCall) (ToolResponse, error) {
	...
	if totalLines == 0 {
		if input.StartLine != 1 || input.EndLine != 1 {
			return NewTextErrorResponse("Document is empty. To create the first draft, replace line 1 through line 1 with new_content."), nil
		}
		if strings.TrimSpace(input.NewContent) == "" {
			return NewTextErrorResponse("new_content is required when creating content in an empty document."), nil
		}
		...
		if t.draftSaver != nil {
			...
			if articleID != "" {
				if err := t.draftSaver.UpdateDraftContent(ctx, articleID, newMarkdown); err != nil {
					log.Printf("   ⚠️ [ReplaceLines] Failed to persist initial draft: %v", err)
				} else {
					log.Printf("   💾 [ReplaceLines] Initial draft persisted to DB")
				}
			}
		}
