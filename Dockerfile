...
RUN cargo build --locked --release --bin blog-backend --bin migrate --bin seed --bin stamp-diesel-migrations --bin schema-fingerprint --bin external-fixtures
...
COPY --from=builder /app/backend/target/release/stamp-diesel-migrations /usr/local/bin/stamp-diesel-migrations
