...
pub struct PageAuthenticated;

impl<S> FromRequestParts<S> for PageAuthenticated
where
    S: Send + Sync,
    AuthState: FromRef<S>,
{
    type Rejection = PageApiError;

    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        let header = parts
            .headers
            .get(AUTHORIZATION)
            .ok_or_else(|| PageApiError::unauthorized("Not authenticated"))?
            .to_str()
            .map_err(|_| PageApiError::unauthorized("Invalid token format"))?;
        if !header.starts_with("Bearer ") {
            return Err(PageApiError::unauthorized("Invalid token format"));
        }
