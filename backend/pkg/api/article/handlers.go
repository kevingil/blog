...
// @Summary Start a new generation chat session
// @Description Creates a draft article shell and kicks off the first agent chat turn against it. Returns the article and a requestId the client can subscribe to over WebSocket for streaming output.
// @Tags articles
// @Accept json
// @Produce json
// @Param request body object{prompt=string,title=string} true "Generation request"
// @Success 200 {object} response.SuccessResponse{data=object{article=dto.ArticleResponse,request_id=string}}
...
func GenerateArticle(c *fiber.Ctx) error {
	...
	if prompt == "" && title == "" {
		return response.Error(c, core.InvalidInputError("Either title or prompt is required"))
	}
	...
	generationMessage := prompt
	if prompt == "" {
		generationMessage = "Write a complete blog article about: " + title
	} else if title != "" {
		generationMessage = "Title: " + title + "\n\n" + prompt
	}
	...
	svc := getService()
	article, err := svc.CreateDraftShell(c.Context(), draftTitle, userID)
	if err != nil {
		return response.Error(c, err)
	}

	requestID, err := copilot.GetManager().SubmitChatRequest(copilot.ChatRequest{
